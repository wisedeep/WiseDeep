...
import { MessageCircle, Send, User, Calendar, BookOpen, Brain, Settings, ArrowLeft } from "lucide-react";
...
const UserMessages = () => {
    ...
    const navItems = [
        { icon: <Calendar className="w-5 h-5" />, label: "Dashboard", path: "/dashboard" },
        { icon: <BookOpen className="w-5 h-5" />, label: "Courses", path: "/courses" },
        { icon: <Calendar className="w-5 h-5" />, label: "My Sessions", path: "/sessions" },
        { icon: <MessageCircle className="w-5 h-5" />, label: "Messages", path: "/messages" },
        { icon: <Brain className="w-5 h-5" />, label: "AI Chat", path: "/ai-chat" },
        { icon: <Settings className="w-5 h-5" />, label: "Profile", path: "/profile" },
    ];



    // Ref to track selected counsellor ID for socket callback
    const selectedCounsellorIdRef = useRef<string | null>(null);

    useEffect(() => {
        selectedCounsellorIdRef.current = selectedCounsellor?.id || null;
    }, [selectedCounsellor]);
    ...
    useEffect(() => {
        ...
        newSocket.on('receive-message', (message: Message) => {
            ...
            const currentSelectedId = selectedCounsellorIdRef.current;
    ...
    const fetchMessages = async (counsellorId: string) => {
        try {
            const response = await api.get(`/user/messages/${counsellorId}`);
            setMessages(response.data);
            // Update unread count in conversations
            setConversations(prev => prev.map(conv =>
                conv.counsellor.id === counsellorId ? { ...conv, unreadCount: 0 } : conv
            ));
        } catch (error) {
            toast({
                title: "Error",
    ...
    const handleSendMessage = async () => {
        if (!newMessage.trim() || !selectedCounsellor || sending) return;

        setSending(true);
        try {
            await api.post('/user/messages/send', {
                counsellorId: selectedCounsellor.id,
                content: newMessage
            });
            setNewMessage("");
            await fetchMessages(selectedCounsellor.id);
        ...
        <DashboardLayout navItems={navItems} userRole="User">
            <div className="h-[calc(100vh-8rem)] flex flex-col animate-fade-in">
                <div className="mb-4">
                    ...
                    <p className="text-muted-foreground">Chat with your counsellors</p>
                </div>
                        ...
                        <CardContent className="flex-1 overflow-y-auto space-y-2">
                            {loading ? (
                                ...
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-sm text-muted-foreground">Book a session to start messaging</p>
                                </div>
                            ) : (
                                conversations.map((conv) => (
                                    <div
                                        key={conv.counsellor.id}
                                        onClick={() => handleSelectCounsellor(conv.counsellor)}
                                        className={`p-4 rounded-lg cursor-pointer transition-all hover:bg-muted ${selectedCounsellor?.id === conv.counsellor.id ? 'bg-muted border-2 border-primary' : 'border border-border'
                                            }`}
                                    >
                                            ...
                                            <div className="flex items-center gap-3 flex-1">
                                                <div className="w-10 h-10 rounded-full bg-gradient-saffron flex items-center justify-center flex-shrink-0">
                                                    <span className="text-sm font-bold text-primary-foreground">
                                                        {conv.counsellor.name.split(' ').map(n => n[0]).join('')}
                                                    </span>
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <h3 className="font-semibold truncate">{conv.counsellor.name}</h3>
                                                    {conv.counsellor.specialization && (
                                                        <p className="text-xs text-muted-foreground truncate">{conv.counsellor.specialization}</p>
                                                    )}
                                                    {conv.latestMessage && (
                        ...
                        {selectedCounsellor ? (
                            <>
                                <CardHeader className="border-b">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <CardTitle>{selectedCounsellor.name}</CardTitle>
                            ...
                            <CardContent className="flex-1 flex items-center justify-center">
                                <div className="text-center">
                                    ...
                                    <p className="text-muted-foreground">Choose a counsellor to start messaging</p>
                                </div>
                            </CardContent>
                        )}
                    </Card>
